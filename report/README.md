...
`varint`: число делится на группы по 7 бит (little-endian, младшие биты идут первыми). В каждом байте старший бит (8-й)
указывает, есть ли следующие байты:

```text
int32 300 = 100101100
0000010 | 0101100  → 2 и 44 (в десятичной).
Первый байт: 44 + флаг продолжения (1 в 8-м бите): 10101100 → 0xAC (hex)
Второй байт: 2 без флага (0 в 8-м бите): 00000010 → 0x02.
Итог: 0xAC 0x02
```
...
Встроенной реализации Circuit Breaker нет ни в HTTP/REST ни в gRPC, но ее можно добавить программно с
